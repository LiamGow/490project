...
class StyleTransfer(Operation):
    ...
    def apply(self, img, iterations):
        ...
        model = self.network(input_tensor=input_tensor).model
        ...
        def total_variation_loss(x):
            ...
            if K.image_data_format() == 'channels_first':
                a = K.square(x[:, :, :img_nrows - 1, :img_ncols - 1] - x[:, :, 1:,
                                                                       :img_ncols - 1])
                b = K.square(x[:, :, :img_nrows - 1, :img_ncols - 1] - x[:, :,
                                                                       :img_nrows - 1,
                                                                       1:])
            else:
                a = K.square(x[:, :img_nrows - 1, :img_ncols - 1, :] - x[:, 1:,
        ...
        loss += self.args.content_weight * content_loss(base_image_features,
        ...
        for layer_name in feature_layers:
            ...
            loss += (self.args.style_weight / len(feature_layers)) * sl
        loss += self.args.variation * total_variation_loss(combination_image)
        ...
        return x.reshape(1, img_ncols, img_nrows, 3)
